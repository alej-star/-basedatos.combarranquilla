...
class Dashboard(tk.Tk):
    ...
    def _stat_card(self, parent, icon, label, value, color):
        ...
        tk.Label(
            card, text=icon, font=("Segoe UI Emoji", 16), bg=color
    ...
    def refresh(self):
        ...
        self._stat_card(
            self.cards_frame, "👥", "USUARIOS", n_usuarios, "#cfe8ff"
        )
        self._stat_card(self.cards_frame, "📚", "LIBROS", n_libros, "#d6f5d6")
        self._stat_card(
            self.cards_frame, "✍️", "AUTORES", n_autores, "#ffe8b3"
        )
        self._stat_card(
            self.cards_frame, "🔄", "PRÉSTAMOS", n_prestamos, "#e6ccff"
        )

...
class LibrosWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Button(
            btns, text="Limpiar", width=12, command=self._limpiar
        ).pack(side="left", padx=5)

        # Listado
        list_frame = tk.LabelFrame(
            self, text="LISTADO DE LIBROS", font=("Segoe UI", 10, "bold")
        )
        ...
        self.total_label = tk.Label(
            self, text="Total libros: 0", font=("Segoe UI", 9, "bold")
        )
    ...
    def _on_select(self, event):
        ...
        self.autor_combo.set(vals[2] if vals[2] is not None else "")
        self.editorial_combo.set(vals[3] if vals[3] is not None else "")
        self.categoria_combo.set(vals[4] if vals[4] is not None else "")

        self.anio_entry.delete(0, tk.END)
        self.anio_entry.insert(0, vals[5] if vals[5] is not None else "")
        ...
        self.isbn_entry.insert(
            0, row["isbn"] if row and row["isbn"] is not None else ""
        )

    def _get_ids(self):
        autor_id = next(
            (a["id"] for a in self.autores if a["nombre"] == self.autor_combo.get()),
            None,
        )
        editorial_id = next(
            (
    ...
    def _guardar(self):
        ...
        try:
            ...
            log_auditoria(
                "libros", "INSERT", new_id, f"Libro creado: {titulo}"
            )
            ...
            messagebox.showerror(
                "Error", f"No se pudo guardar (¿ISBN duplicado?): {e}"
            )
        finally:
            conn.close()
    ...
    def _actualizar(self):
        if not self.selected_id:
            messagebox.showwarning(
                "Selección", "Seleccione un libro de la lista."
            )
            return
    ...
    def _eliminar(self):
        ...
        if not messagebox.askyesno(
            "Confirmar", "¿Está seguro de eliminar este libro?"
        ):
            return
        ...
        try:
            cur.execute(
                "SELECT titulo FROM libros WHERE id=?", (self.selected_id,)
            )
            row = cur.fetchone()
            ...
            messagebox.showerror(
                "Error",
                f"No se pudo eliminar (verifique préstamos asociados): {e}",
            )
        finally:
            conn.close()
